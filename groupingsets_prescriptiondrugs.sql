...
--2. Now, let's say that we want our output to also include the total number of claims 
...
--4 largest cities in Tennessee (Nashville, Memphis, Knoxville, and Chattanooga), 
...
--For example, we could count both of 
--"ACETAMINOPHEN WITH CODEINE" and "CODEINE SULFATE" as being "CODEINE" for the purposes of this question.
...
SELECT *,
	CASE WHEN generic_name LIKE '%HYDROCODONE%' THEN 'hydrocodone'
		...
		ELSE 'not_included' END AS common_opioid_type
FROM drug;
...
--Hint #2: In order to use the crosstab function, you need to first write a query 
--which will produce a table with one row_name column, one category column, and one value column. 
...
--If the query that you are using also uses single quotes, you'll need to 
